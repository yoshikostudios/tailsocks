...
class TestTailscaleProxyManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_expanduser.side_effect = lambda path: path.replace('~', self.temp_dir.name)
    ...
    def test_init_creates_directories(self):
        ...
        config_dir = os.path.join(self.temp_dir.name, f".config/tailscale-{self.profile_name}")
        cache_dir = os.path.join(self.temp_dir.name, f".cache/tailscale-{self.profile_name}")
    ...
    def test_init_creates_default_config(self):
        ...
        config_path = os.path.join(self.temp_dir.name, f".config/tailscale-{self.profile_name}/config.yaml")
    ...
    def test_get_status(self, mock_run):
        ...
        mock_process = MagicMock()
        mock_process.returncode = 0
        mock_process.stdout = '{"BackendState": "Running", "Self": {"TailscaleIPs": ["100.100.100.100"]}}'
        mock_run.return_value = mock_process
...
class TestGetAllProfiles(unittest.TestCase):
    ...
    def test_get_all_profiles(self, mock_manager_class, mock_glob):
        ...
        mock_glob.side_effect = [
            ['/home/user/.config/tailscale-profile1', '/home/user/.config/tailscale-profile2'],
            ['/home/user/.cache/tailscale-profile1', '/home/user/.cache/tailscale-profile3']
        ]
